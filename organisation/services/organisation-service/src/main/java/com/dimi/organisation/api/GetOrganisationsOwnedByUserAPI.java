...
@RestController
@Validated
public class GetOrganisationsOwnedByUserAPI
{
    @Autowired private OrganisationService organisationService;


    @GetMapping(value = "/organisations/owners/{ownerUserID}")
    public ResponseEntity<APIResponse> getOrganisations(@PathVariable(name = "ownerUserID") UUID ownerUserID)
    {
        List<Organisation> organisations = new ArrayList<>();
        organisationService.getAllByOwnerUserID(ownerUserID).forEach(o -> organisations.add(Organisation.builder()
